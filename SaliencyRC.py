...
def getMainLine(img, mask_x, mask_y, mask_height, mask_width, sign):
    img_height = img.shape[0]
    img_width =img.shape[1]
    mask_img = np.zeros((img_height, img_width), np.uint8)
    for row in range(mask_y, img_height if (mask_y + mask_height > img_height)  else (mask_y + mask_height)):
        for col in range(mask_x , img_width if (mask_y + mask_width > img_width)  else (mask_y + mask_width)):
            mask_img[row, col] = 1
    ...
    if(sign == -1):
        ...
        ret, binary = cv2.threshold(img, sign, 255, cv2.THRESH_BINARY)
    # ret1, binary1 = cv2.threshold(sal,0,255,cv2.THRESH_BINARY | cv2.THRESH_TRIANGLE)
    # ret, binary = cv2.threshold(sal, 40, 255, cv2.THRESH_BINARY)
    # ret1, binary1 = cv2.threshold(sal, 50, 255, cv2.THRESH_BINARY)
    # 经验参数阈值为50，三角形法获取阈值效果较好(面积变小之后会有改变)
...
#    print(len(contours))
    ...
    for line in contours:
        ...
        if(area > max):
            max = area
            line_max = line
#    print("testtest",line_max)
#    print("threshold value is :", ret)
    line_simple = cv2.approxPolyDP(line_max, 0.002*cv2.arcLength(line_max,True), True)
    return line_simple,binary

def saveLine(line):
    line_dict = {}
    line_dict['name'] = 'imagename2'
    line_dict['content'] = line.tolist()
    json_data = json.dumps(line_dict)
    line = line.tolist()
    os.chdir(r"D:\SaliencyRC\SaliencyRC")
    with open("lineDict.json", 'r') as f:
        old_data = json.load(f)
        old_data.update(line_dict)
        # old_data = ChainMap(old_data, line_dict)
        print(old_data)
    with open("lineDict.json", 'w') as f:
